...
void leerMatrizCramer(Matriz*, int l, int k,float* indep);
...
void resolverSitemasEcuacionesPorGaussJordan(Matriz* matriz1, Matriz* matriz2);
...
void menu(void) {
	...
	do {
		...
		gotoxy(45, 17);
		puts("8-Solucion de sistemas de ecuaciones por el metodo Gauss-Jordan");
		gotoxy(45, 18);
		puts("0-Salir");
		centrarTexto("FLECHA 'ARRIBA' Y 'ABAJO' O 'W' Y 'S'PARA DESPLAZARSE ENTRE OPCIONES Y 'ENTER' PARA SELECCIONAR LA OPCION", 23);
		centrarTexto("NOTA: TAMANIO MATRICES DE MINIMO 1x1 MAXIMO 4x4", 25);
		centrarTexto("Y LAS MATRICES SIGUEN EL FORMATO FILAS COLUMNAS RESPECTIVAMENTE", 26);
		operacion = 1;
		gotoxy(39, 10);
		putchar('>');
		...
		case 7:
			...
			break;
		case 8:
			resolverSitemasEcuacionesPorGaussJordan(&matriz1, &matriz2);
			break;
		case 9:
			operacion = 0;
			limpiarPantalla();
...
void leerMatrizCramer(Matriz* matriz, int x, int y, float* indep) {
	...
	for (int row = 0, k = y; row < matriz->filas; row++, k++) {
		...
		gotoxy(l+matriz->columnas*7+3, k);
		scanf("%f", &indep[row]);
		putchar('\n');
	}
}
