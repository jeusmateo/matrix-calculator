...
void leerMatriz(Matriz* matriz) {
    ...
    for (int row = 0, k = 15; row < matriz->filas; row++, k++) {
        for (int col = 0, l = 49; col < matriz->columnas; col++, l += 7) {
            gotoxy(l, k);scanf("%f", &matriz->datos[row][col]);
            moverCursorArriba(1);
...
void imprimirMatriz(Matriz* matriz) {
    for (int row = 0, k = 15; row < matriz->filas; row++, k++) {
        for (int col = 0, l = 83; col < matriz->columnas; col++, l += 7) {
            gotoxy(l, k);printf("%g\t", matriz->datos[row][col]);
        }
        putchar('\n');
    }
}
...
void recuadro(int xs, int ys, int xi, int yi) {
    ...
    for (i = ys; i <= yi; i++) {
        gotoxy(xs - 1, i - 1); putchar(179);
        gotoxy(xi - 1, i - 1); putchar(179);
    }
    
    for (i = xs; i <= xi; i++) {
        gotoxy(i - 1, ys - 1); putchar(196);
        gotoxy(i - 1, yi - 1); putchar(196);
    }
    
    gotoxy(xs - 1, ys - 1); putchar(218);
...
void centrarTexto(char* texto, int y) {
    ...
    gotoxy(60 - (tamanio / 2), y); puts(texto);
}
